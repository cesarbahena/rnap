...
Histones answer what governance fact exists. eRNA is `NormalizedArtifact::Executable`: a human-readable executable workflow-definition artifact with IAM-like DSL attributes. eRNA defines workflows across DNAp, including lifecycle gates, command/state transitions, dependency requirements, and governance checks as those workflows are approved.
